...
const rooms = {
    ...
    2: {
        members: [ "5c124e752280a7dfb5f5f980", "5c1229362280a7dfb5f5f97f"]
    }
}
...
io.on('connection', socket => {
    ...
    socket.use((packet, next) => {
        ...
        if (!message || !room) return next(new Error('missile fields (message or room)'));

        if (!rooms.hasOwnProperty(room)) return next(new Error('Room does not exist'));

        if (rooms[room].members.length <= 1) return next(new Error('message sent to empty room'));

        const sender = clients[socket.id]._id;

        if (!rooms[room].members.find(item => item === sender)) return next(new Error('Sender is not in room'));
...
const handleMessage = (message, { id }) => {
    ...
    database.addMessage(message, id)
    .then(res => {
...
const userConnect = ({ id }, { username, _id }) => {
    ...
    queue.get(_id)
    .then(res => {
        io.to(id).emit('sendMessageToClients', res)
    })
